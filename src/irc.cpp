...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #Nanotec-TESTNET\r");
            Send(hSocket, "WHO #Nanotec-TESTNET\r");
        } else {
            ...
            Send(hSocket, strprintf("JOIN #Nanotec-REALNET\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Nanotec-REALNET\r", channel_number).c_str());
        }
