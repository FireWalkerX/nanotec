...
unsigned int nStakeMinAge = 1 * nOneHour;       //bazmod_minstakeageof1hour 
unsigned int nStakeMaxAge = 30 * 24 * nOneHour; //bazmod_maxstakeageof30days
unsigned int nStakeTargetSpacing = 2.5 * 60;    //bazmod_2.5minstaketime
int64_t nTargetSpacing = 2.5 * 60;              //bazmod_2.5minblocktime
...
const string strMessageMagic = "Vote Nanotec for president!\n";
...
// staking reward based on 'ladder' position (baz)
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nCoinValue, unsigned int nBits, int64_t nTime)
{
    int64_t nRewardCoinYear = GetLadderReward(nCoinValue);
    // below formula is the most accurate full year integer
    int64_t nSubsidy = nCoinAge * nRewardCoinYear * 33 / (365 * 33 + 8);
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): LadderRung=%.1f create=%s nCoinAge=%s nCoinValue=%s \n", (double)nRewardCoinYear/(double)CENT, FormatMoney(nSubsidy).c_str(), nCoinAge, FormatMoney(nCoinValue).c_str());
    return nSubsidy;
}

// determine which 'ladder' position we are in (baz)
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	if (nActualSpacing > 3 * (5*60))
		nActualSpacing = 3 * (5*60);
	...
	if (pindexLast->nHeight < 2200)
	{
	nInterval = nTargetTimespan / (5*60);
    bnNew *= ((nInterval - 1) * (5*60) + nActualSpacing + nActualSpacing);
    bnNew /= ((nInterval + 1) * (5*60));
	} else {
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            if (nTime >  Checkpoints::GetLastCheckpointTime())
            {
                ...
                int64_t nCalculatedReward = GetProofOfStakeReward(nCoinAge, nCoinValue, pindexBlock->nBits, nTime) - GetMinFee(1, false, GMF_BLOCK, nTxSize) + CENT;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Nanotec
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Nanotec: check proof-of-stake block signature
...
{
    unsigned int template_ntime;
    unsigned int template_nonce;
    unsigned int template_inittxtime;
 
    if (fTestNet)
    {
        pchMessageStart[0] = 0x4E; // NNCT (NNC TestNet)
    ...
    if (mapBlockIndex.empty())
    {
        ...
        if (fTestNet){
           //testnet
           template_ntime = 1470257731;
           template_nonce = 303218;
           template_inittxtime = template_ntime - 1;
        } else {
           //realnet
           template_ntime = 1470257730;
           template_nonce = 539440;
           template_inittxtime = template_ntime - 1;
        }

        const char* pszTimestamp = "NNC Genesis Block 04082016";
        CTransaction txNew;
        txNew.nTime = template_inittxtime;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = template_ntime;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = template_nonce; 

        // set to true to mine genesisblock
        ...
        //// debug print
        if(!fTestNet){
         assert(block.hashMerkleRoot == uint256("0x179449b392875b650f2bc93804c2f305d3c37196f4587d022e6e52a038012e90"));
        } else {
         assert(block.hashMerkleRoot == uint256("0x8937ce2bdf1f4a5638d5d5a0768b46ad93e8c14bde01605609cf2cb6c83112b5")); 
        }
...
unsigned char pchMessageStart[4] = { 0x4E, 0x4E, 0x43, 0x52 }; // NNCR (NNC Realnet)
